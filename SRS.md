...
本文档旨在定义 **Ophanim Simulator** 的详细软件需求。该系统是一个面向科研的高保真卫星星座仿真平台，旨在支持 **轨道边缘计算 (Orbital Edge Computing)**、**星间链路网络 (ISL)** 及 **智能协同算法 (如 RL, Bend-pipe)** 的验证与可视化。
...
| 序列 | 名称 | 描述 | 输入 | 输出 |
| --- | --- | --- | --- | --- |
| FR-PHY-01 | 轨道传播 | 系统必须能够读取 TLE 数据，并基于 SGP4 算法计算任意给定时间点的卫星位置与速度。 | TLE 文件, 仿真时间戳 。 | 卫星在 J2000 (ECI) 和 ECEF 坐标系下的位置向量  和速度向量 。 |
| FR-PHY-02 | 覆盖性分析 | 系统需根据卫星载荷的视场角 (FOV) 和姿态，计算其对地面的覆盖区域。 | 卫星视锥体 (Sensor Cone)坐标 | 目标是否可见 |
| FR-PHY-03 | 光照分析 | 系统需根据太阳位置计算卫星是否处于阴影区 (Eclipse)。 | 太阳位置与卫星位置坐标 | 否处于阴影区 (Eclipse) |
| FR-PHY-04 | 电池状态 | 系统需根据卫星是否处于阴影区 (Eclipse)，并据此切换充电/放电状态。 | 否处于阴影区 (Eclipse) | 充电/放电状态 |
| FR-PHY-05 | 星座生成器 | 系统应支持用户通过输入 Walker 星座参数自动生成大规模虚拟星座。 | Walker 星座参数 | 虚拟 TLE |

### 3.2 网络与通信 (Network Layer)

| 序列 | 名称 | 描述 | 输入 | 输出 |
| --- | --- | --- | --- | --- |
| FR-NET-01 | ISL动态拓扑生成 | 在每个仿真步长，系统需计算所有卫星对之间的距离，然后判断连接状态。 | Euc 和 LoS 距离 | ISL连接索引表 |
| FR-NET-02 | SGL动态拓扑生成 | 在每个仿真步长，系统需计算所有卫星-基站对之间的仰角，然后判断连接状态。 | 仰角 | SGL连接索引表 |
| FR-NET-03 | 数据传输模拟 | 模拟数据包在链路上的传输，计算传输延迟 (Propagation Delay) 和传输时间 (Transmission Delay)。 | - | - |

### 3.3 边缘计算与资源 (Computing Layer)

| 序列 | 名称 | 描述 | 输入 | 输出 |
| --- | --- | --- | --- | --- |
| FR-COMP-01 | 任务队列管理 | 任务不仅占用存储空间，处理任务时还需占用 CPU 时间并消耗额外电量 | - | - |
| FR-COMP-02 | 电池模型 | 电量耗尽时，卫星进入“失效”或“休眠”状态，无法执行任务。 | - | - |

### 3.4 算法接口与控制 (Algorithm Interface)

| 序列 | 名称 | 描述 | 输入 | 输出 |
| --- | --- | --- | --- | --- |
| FR-ALG-01 | 状态观测 | 系统提供 API 返回当前的全局或局部状态 。 | - | JSON 格式，包含位置、电量、邻居列表、任务队列长度 |
| FR-ALG-02 | 动作执行 | 系统接收算法输出的动作向量  并应用到仿真环境。 | - | 动作空间 |
